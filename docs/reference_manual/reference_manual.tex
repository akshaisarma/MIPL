...
algorithms and retrieve data from the computation.

Also algoritm developers can benefit from MIPL's Job decription interface,
so that their algorithms are automatically distributed and accelerated
on the computer clusters and hardware accelerators.
...
\subsection {Term}
Term is the only one data type for Facts, Rules, and Querys.  Numbers, Strings,
FRQs and even Terms are recursively defined in a form of Term. Variables can
contain a Term.
\medskip

\subsection {Job Types}
Unlike FRQ part of MIPL, a Job description has two primitive types, Matrix and
Number.  Matrix and Number can be computed within a Job description. Variables
can contain a Number or a Matrix.
