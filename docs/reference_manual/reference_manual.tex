...
$fact$ is ended with $"."$. $fact$ is used to describe or define the fact. $fact$ should be started with the a small letter. Also, several $fact$s can be combined in one line with $,$.
...
$query$ is ended with $"?"$. $query$ requires facts, or job results. $query$ with the fact returns $true$ or $false$. $query$ with the job results returns satisfying results about the query. If the job results have lots of columns, $query$ can take $*$ as the query argument.
